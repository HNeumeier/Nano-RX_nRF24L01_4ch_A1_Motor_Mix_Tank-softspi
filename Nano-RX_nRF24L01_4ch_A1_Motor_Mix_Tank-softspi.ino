...
void output_PWM()
{
  ...
  if (mix1 > MID_CONTROL_VAL + DEAD_ZONE)
  {
    ...
    #ifdef DEBUG_SERIAL
      Serial.print(" FmotAmotA: "); //print value ​​on a serial monitor
  ...
  else if (mix1 < MID_CONTROL_VAL - DEAD_ZONE)
  {
    ...
    #ifdef DEBUG_SERIAL
      Serial.print(" BmotAmotA: "); //print value ​​on a serial monitor
      Serial.println(value_motorA); //print value ​​on a serial monitor
    #endif
  }
  else
  ...
  if (mix2 > MID_CONTROL_VAL + DEAD_ZONE)
  {
    ...
    #ifdef DEBUG_SERIAL
      Serial.print(" FmotAmotB: "); //print value ​​on a serial monitor
...
unsigned long empfangszeit = 0;

void last_rx_time()
{
  empfangszeit = millis() - rx_time;
  if (empfangszeit < 1)   telemetry_packet.rssi = 100;
  if (empfangszeit > 1 and empfangszeit < 5) telemetry_packet.rssi = 90;
  if (empfangszeit > 5 and empfangszeit < 10) telemetry_packet.rssi = 50;
  if (empfangszeit > 10) telemetry_packet.rssi = 0;
